...
data_summary <- list('sex' =
                     ...
                     'intubed' = 
                       list('yes (1)' = ~ sum(intubed == 1),
                            'no (2)' = ~ sum(intubed == 2),
                            'not applicable (97)' = ~ sum(intubed == 97),
                            'ignored (98)' = ~ sum(intubed == 98),
                            'not specified (99)' = ~ sum(intubed == 99)),
                     'pneumonia' = 
                       list('yes (1)' = ~ sum(pneumonia == 1),
                            'no (2)' = ~ sum(pneumonia == 2),
                            'not specified (99)' = ~ sum(pneumonia == 99)),
                     'age' = 
                       list('mean' = ~ round(mean(age),2),
                            'median' = ~ median(age),
                            'standard deviation' = ~ round(sd(age),2)),
                     'pregnancy' = 
                       list('yes (1)' = ~ sum(pregnancy == 1),
                            'no (2)' = ~ sum(pregnancy == 2),
                            'not applicable (97)' = ~ sum(pregnancy == 97),
                            'ignored (98)' = ~ sum(pregnancy == 98)),
                     'diabetes' = 
                       list('yes (1)' = ~ sum(diabetes == 1),
                            'no (2)' = ~ sum(diabetes == 2),
                            'ignored (98)' = ~ sum(diabetes == 98)),
                     'copd' = 
                       list('yes (1)' = ~ sum(copd == 1),
                            'no (2)' = ~ sum(copd == 2),
                            'ignored (98)' = ~ sum(copd == 98)),
                     'asthma' = 
                       list('yes (1)' = ~ sum(asthma == 1),
                            'no (2)' = ~ sum(asthma == 2),
                            'ignored (98)' = ~ sum(asthma == 98)),
                     'inmsupr' = 
                       list('yes (1)' = ~ sum(inmsupr == 1),
                            'no (2)' = ~ sum(inmsupr == 2),
                            'ignored (98)' = ~ sum(inmsupr == 98)),
                     'hypertension' = 
                       list('yes (1)' = ~ sum(hypertension == 1),
                            'no (2)' = ~ sum(hypertension == 2),
                            'ignored (98)' = ~ sum(hypertension == 98)),
                     'other_disease' = 
                       list('yes (1)' = ~ sum(other_disease == 1),
                            'no (2)' = ~ sum(other_disease == 2),
                            'ignored (98)' = ~ sum(other_disease == 98)),
                     'cardiovascular' = 
                       list('yes (1)' = ~ sum(cardiovascular == 1),
                            'no (2)' = ~ sum(cardiovascular == 2),
                            'ignored (98)' = ~ sum(cardiovascular == 98)),
                     'obesity' = 
                       list('yes (1)' = ~ sum(obesity == 1),
                            'no (2)' = ~ sum(obesity == 2),
                            'ignored (98)' = ~ sum(obesity == 98)),
                     'renal_chronic' = 
                       list('yes (1)' = ~ sum(renal_chronic == 1),
                            'no (2)' = ~ sum(renal_chronic == 2),
                            'ignored (98)' = ~ sum(renal_chronic == 98)),
                     'tobacco' = 
                       list('yes (1)' = ~ sum(tobacco == 1),
                            'no (2)' = ~ sum(tobacco == 2),
                            'ignored (98)' = ~ sum(tobacco == 98)),
                     'contact_other_covid' = 
                       list('yes (1)' = ~ sum(contact_other_covid == 1),
                            'no (2)' = ~ sum(contact_other_covid == 2),
                            'not specified (99)' = ~ sum(contact_other_covid == 99)),
                     'icu' = 
                       list('yes (1)' = ~ sum(icu == 1),
                            'no (2)' = ~ sum(icu == 2),
                            'not applicable (97)' = ~ sum(icu == 97),
                            'not specified (99)' = ~ sum(icu == 99)))
...
proc_NA <- function(df){
  ...
  df[, names(df) %in% precondition_cols][df[, names(df) %in% precondition_cols] == 'Ignored (98)'] <- NA
  df[, names(df) %in% precondition_cols][df[, names(df) %in% precondition_cols] == 97] <- NA
  df[, names(df) %in% precondition_cols][df[, names(df) %in% precondition_cols] == 'Not applicable (97)'] <- NA
...
```
```{r plot missing data, warning=FALSE, echo=FALSE}
...
p2 <- ggplot(covid_complete_case,aes(x=pneumonia, fill=deathyn,alpha=0.3)) + geom_bar(position='fill') + scale_fill_manual(values=c('white','red')) + scale_x_discrete(labels = c('No (2)','Diagnosed')) + ylab("") +  xlab("") + ggtitle("Pneumonia") + theme(plot.title=element_text(hjust=0.5,size=10), legend.position = 'none') + ylim(0,0.5)
p3 <- ggplot(covid_complete_case,aes(x=diabetes, fill=deathyn,alpha=0.3)) + geom_bar(position='fill') + scale_fill_manual(values=c('white','red')) + scale_x_discrete(labels = c('No (2)','Diagnosed')) + ylab("") + xlab("") +  ggtitle("Diabetes") + theme(plot.title=element_text(hjust=0.5,size=10), legend.position = 'none') + ylim(0,0.5)
p4 <- ggplot(covid_complete_case,aes(x=copd, fill=deathyn,alpha=0.3)) + geom_bar(position='fill') + scale_fill_manual(values=c('white','red')) + scale_x_discrete(labels = c('No (2)','Diagnosed')) + ylab("") + xlab("") +  ggtitle("COPD") + theme(plot.title=element_text(hjust=0.5,size=10), legend.position = 'none') + ylim(0,0.5)
p5 <- ggplot(covid_complete_case,aes(x=asthma, fill=deathyn,alpha=0.3)) + geom_bar(position='fill') + scale_fill_manual(values=c('white','red')) + scale_x_discrete(labels = c('No (2)','Diagnosed')) + ylab("") + xlab("") +  ggtitle("Asthma") + theme(plot.title=element_text(hjust=0.5,size=10), legend.position = 'none') + ylim(0,0.5)
p6 <- ggplot(covid_complete_case,aes(x=inmsupr, fill=deathyn,alpha=0.3)) + geom_bar(position='fill') + scale_fill_manual(values=c('white','red')) + scale_x_discrete(labels = c('No (2)','Diagnosed')) + ylab("") +  xlab("") + ggtitle("Inmunosuppressed") + theme(plot.title=element_text(hjust=0.5,size=10), legend.position = 'none') + ylim(0,0.5)
p7 <- ggplot(covid_complete_case,aes(x=hypertension, fill=deathyn,alpha=0.3)) + geom_bar(position='fill') + scale_fill_manual(values=c('white','red')) + scale_x_discrete(labels = c('No (2)','Diagnosed')) + ylab("") + xlab("") +  ggtitle("Hypertension") + theme(plot.title=element_text(hjust=0.5,size=10), legend.position = 'none') + ylim(0,0.5)
p8 <- ggplot(covid_complete_case,aes(x=other_disease, fill=deathyn,alpha=0.3)) + geom_bar(position='fill') + scale_fill_manual(values=c('white','red')) + scale_x_discrete(labels = c('No (2)','Diagnosed')) + ylab("") +  xlab("") + ggtitle("Other Diseases") + theme(plot.title=element_text(hjust=0.5,size=10), legend.position = 'none') + ylim(0,0.5)
p9 <- ggplot(covid_complete_case,aes(x=cardiovascular, fill=deathyn,alpha=0.3)) + geom_bar(position='fill') + scale_fill_manual(values=c('white','red')) + scale_x_discrete(labels = c('No (2)','Diagnosed')) + ylab("") + xlab("") +  ggtitle("Cardiovascular") + theme(plot.title=element_text(hjust=0.5,size=10), legend.position = 'none') + ylim(0,0.5)
p10 <- ggplot(covid_complete_case,aes(x=obesity, fill=deathyn,alpha=0.3)) + geom_bar(position='fill') + scale_fill_manual(values=c('white','red')) + scale_x_discrete(labels = c('No (2)','Diagnosed')) + ylab("") + xlab("") +  ggtitle("Obesity") + theme(plot.title=element_text(hjust=0.5,size=10), legend.position = 'none') + ylim(0,0.5)
p11 <- ggplot(covid_complete_case,aes(x=renal_chronic, fill=deathyn,alpha=0.3)) + geom_bar(position='fill') + scale_fill_manual(values=c('white','red')) + scale_x_discrete(labels = c('No (2)','Diagnosed')) + ylab("") + xlab("") +  ggtitle("Kidney Disease") + theme(plot.title=element_text(hjust=0.5,size=10), legend.position = 'none') + ylim(0,0.5)
p12 <- ggplot(covid_complete_case,aes(x=tobacco, fill=deathyn,alpha=0.3)) + geom_bar(position='fill') + scale_fill_manual(values=c('white','red')) + scale_x_discrete(labels = c('No (2)','Yes (1)')) + ylab("") + xlab("") +  ggtitle("Tobacco Use") + theme(plot.title=element_text(hjust=0.5,size=10), legend.position = 'none') + ylim(0,0.5)
```
...
\
```{r Select the data, include=FALSE}
...
```{r Make Table}
kable(data.frame('model' = c('original','down','ROSE','SMOTE'), 'Accuracy'=c('88.29%','81.10%','91.39%','84.06%'), 'Specificity' = c('96.70%','79.16%','90.33%','84.99%'), 'Sensitivity'=c('27.96%', '83.05%','92.45%','82.82%'), 'Balanced Accuracy'=c('62.44%','81.11%','91.39%','83.90%')))
```
...
#### Model Selection\
\
We created four different models for our project available and we selected the best one for deployment.
As you can observe from the table, the random forest model has the best performance, as its accuracy is 91.4% which is high. Also, it has the highest F1 score. The other three models have good performance as well, but all have some limitations, as described in each model above. However, unfortunately we can’t use the random forest model for deployment due to the size of the model, as the calculation will take too long if we use it. Therefore, we chose the second best model, the logistic regression model, for deployment. The logistic regression model has an 88.7% accuracy and 91.2% specificity, which is a good choice for deployment.
