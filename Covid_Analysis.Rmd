...
data_summary <- list('sex' =
                       list('male' = ~ count(covid$sex,vars=2),
                            'female' = ~ count(covid$sex,vars=1)))
```
...
true_positives <- c(84.7,85,98609,84.8)
false_positives <- c(8.2,8.5,10553,8.3)
false_negatives <- c(3.1,2.8,8237,3)
true_negatives <- c(4,3.7,100876,3.9)

model_performance <- model_performance %>% mutate(tp = true_positives, fps=false_positives, fn=false_negatives,tn=true_negatives)
...
model_performance <- model_performance %>% mutate(accuracy = (model_performance$tp + model_performance$tn)/model_performance$total, sensitivity = model_performance$tp/(model_performance$tp+model_performance$fn), specificity = model_performance$tn/(model_performance$tn + model_performance$fp), precision = model_performance$tp/(model_performance$tp + model_performance$fp))

model_performance <- model_performance %>% mutate(balanced_accuracy =(model_performance$sensitivity + model_performance$specificity)/2, f1 = 2*model_performance$tp/(2*model_performance$tp + model_performance$fp + model_performance$fn))
